...
export class LaserPool {
  constructor(scene, max = 80) {
    ...
    const matEmpire = new THREE.MeshBasicMaterial({ color: new THREE.Color(0.5, 3.0, 0.3) });
    for (let i = 0; i < max; i++) {
      const mesh = new THREE.Mesh(geo, matPlayer);
      mesh.visible = false;
      scene.add(mesh);
      this.pool.push({
        mesh, matPlayer, matPirate, matPolice, matEmpire,
  ...
  fire(origin, dir, owner, damage, inheritVel) {
    ...
    b.damage = damage;
    b.life = C.LASER_LIFE;
    b.mesh.material = owner === 'player' ? b.matPlayer
      : owner === 'police' ? b.matPolice
      : owner === 'empire' ? b.matEmpire : b.matPirate;
